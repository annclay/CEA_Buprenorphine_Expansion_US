...
discCosts <- read.csv('', header = T)
discCostsPSA <- read.csv('', header = T) #Use the PSA discCosts
discCriminalCosts <- read.csv('', header = T)
discQALYs <- read.csv('', header = T)

###Upload basecase CEA run ### Only needed for status quo (coverage level doesn't matter)
basecaseCEATable <- read.csv("")

FILE_PATH_RESULTS <- ""

if(iReRunAnalysis){
  ###Upload raw sensitivity analysis outputs from source and run the CEA
  filePathSensitivityAnalysis <- ""
  ...
  read_csv_filename <- function(filename){#filename <- df.list[2]
    ...
    editedFileName <- gsub(pattern = "","",x = filename)
  ...
  for(i in list.Interventions){ #i<-list.Interventions[2]
    ...
    for (j in 1:max(interventionModelOutput$Simulation)) { #j=2
      ...
      l_intervention <- list(modelOutput = interventionModelOutput[interventionModelOutput$Simulation == j,],
                             ...
                             #discCosts = discCosts, 
                             discCosts = discCostsPSA[discCostsPSA$nRun == j,],
  ...
  sensAll0 <- read.csv(file = paste0(FILE_PATH_RESULTS,"SensitivityResults",coveragePercent,"cov","2023-03-02.csv"))
...
sensAll <- sensAll1 %>%
  mutate(totalCosts = as.numeric(totalCosts),
         totalQALYS = as.numeric(totalQALYS),
         totalODdeaths = as.numeric(totalODdeaths))%>%
  mutate(IncrementalCosts= totalCosts-statusQuoCosts,
         ...
         NMBwtp = IncrementalQALYs*wtp - IncrementalCosts,
         NMB50 = IncrementalQALYs*50000 - IncrementalCosts,
         NMB100 = IncrementalQALYs*100000 - IncrementalCosts,
         NMB200 = IncrementalQALYs*200000 - IncrementalCosts)
...
if(SaveOutputs){
  ...
  ##All
    ...
    #Summary
  write.csv(sensAllSummary, file = paste0(FILE_PATH_RESULTS,"sensAllSummaryPSA",coveragePercent,"cov",date.run,".csv"))
  write.csv(sensAllSummaryCEACwtp, file = paste0(FILE_PATH_RESULTS, "sensAllSummaryPSAceac",coveragePercent,"cov",date.run,".csv"))
  write.csv(sensAllceacHighestNMB, file = paste0(FILE_PATH_RESULTS, "sensAllSummaryPSAceacHighestNMB",coveragePercent,"cov",date.run,".csv"))
}
